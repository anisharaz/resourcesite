...
async function Department_page({ params }) {
  ...
  const userDepartments = UserDBData?.department.filter(
    (department) => department.program_name === program_name,
  );
  ...
  return (
    <div>
      ...
      {userDepartments?.map((department) => {
        return (
          <Link
            href={`${program_name}/${department.department_code}`}
            key={v4()}
